...
export const useLifecyclePage = () => {
  ...
  const expiringDocs = documents.filter(document => document.status === "expiring_soon").length;
  const activeExits = exitProcesses.filter(process => process.status !== "completed" && process.status !== "cancelled").length;
  const probationAlerts = contracts.filter(contract => {
