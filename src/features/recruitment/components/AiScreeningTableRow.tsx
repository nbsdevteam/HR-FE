...
const AiScreeningTableRow = ({ app, index, onSelect, onUpdateStage }: AiScreeningTableRowProps) => {
  ...
  const handleStageChange = useCallback((stage: string): void => {
  ...
  return (
    ...
    >
      ...
      <td className="px-4 py-3">
        <button onClick={handleSelect} className="text-start cursor-pointer hover:text-primary transition-colors">
      ...
      <td className="px-4 py-3">
        <div className="flex flex-wrap gap-1 max-w-[240px]">
          {(app.matched_skills || []).slice(0, 3).map((skill, si) => (
            <Chip key={`${skill}-${si}`} label={skill} variant="matched" />
          ))}
