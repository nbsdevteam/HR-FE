...
const ApplicantFormQualificationsSection = ({
  education, experienceYears, currentCompany, source, skills, onFieldChange,
  ...
  <fieldset className="rounded-xl border border-border/30 p-4 space-y-4">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div>
        ...
        <select value={education} onChange={e => onFieldChange("education", e.target.value)} className={selectCls}>
      ...
      <div>
        ...
        <input type="number" min={0} max={50} value={experienceYears}
          onChange={e => onFieldChange("experience_years", Number(e.target.value))}
          className={inputCls} dir="ltr" />
      </div>
      <div>
        <label className={labelCls} style={{ fontSize: 13 }}>{arabicSource("common.current_company")}</label>
        <input type="text" value={currentCompany} onChange={e => onFieldChange("current_company", e.target.value)}
      ...
      <div>
        ...
        <select value={source} onChange={e => onFieldChange("source", e.target.value)} className={selectCls}>
    ...
    <div>
      ...
      <input type="text" value={skills} onChange={e => onFieldChange("skills", e.target.value)}
