...
const OrgStructureManagement = ({ onBack }: OrgStructureManagementProps) => {
  ...
  const handleTreeIncludeArchivedChange = useCallback((e: React.ChangeEvent<HTMLInputElement>): void => {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-gradient-gold">{arabicSource("org_structure.management_title")}</h1>
      ...
      {tab === "tree" && (
        <div className="space-y-3">
          <label className="flex items-center gap-2 text-muted-foreground text-sm cursor-pointer w-fit">
            <input type="checkbox" checked={treeIncludeArchived} onChange={handleTreeIncludeArchivedChange} />
      ...
      {tab === "departments" && <DepartmentsManagementSection metadata={metadata} />}
      {tab === "designations" && <DesignationsManagementSection metadata={metadata} />}
    </div>
  );
};
