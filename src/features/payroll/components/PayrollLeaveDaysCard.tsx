...
const PayrollLeaveDaysCard = ({
  leaveRecs,
  paidLeaveCount,
  unpaidLeaveCount,
}: PayrollLeaveDaysCardProps) => (
    ...
    <div className="flex items-center gap-3 mb-4">
      ...
      <div>
        <h3 className="text-foreground">
          {arabicSource("payroll.approved_vacation_days")}
        </h3>
        <p className="text-muted-foreground" style={{ fontSize: 12 }}>
          {paidLeaveCount > 0 &&
            `${paidLeaveCount} ${arabicSource("payroll.day_with_salary")}`}
          {paidLeaveCount > 0 && unpaidLeaveCount > 0 && " — "}
          {unpaidLeaveCount > 0 && (
            <span className="text-destructive">
              {unpaidLeaveCount}{" "}
              {arabicSource("payroll.day_without_pay_deducted")}
            </span>
          )}
        </p>
      </div>
