...
type ExcuseKey = keyof Omit<ExcuseForm, "note">;
...
const ExcuseModal = ({
  ...
  const record = excuseModal?.record;

  const excuseToggles = useMemo(
    () => [
      {
        key: "late" as const,
        label: arabicSource("attendance.excuse_for_delay"),
        desc: arabicSource("attendance.late_salary_will_not_be_counted"),
        show:
          (record?.lateMinutes || 0) > 0 ||
          record?.status === arabicSource("common.late"),
      },
      {
        key: "shortfall" as const,
        label: arabicSource("attendance.excuse_of_lack_of_hours"),
        desc: arabicSource(
          "attendance.short_hours_for_this_day_will_not_be_deducted",
        ),
        show: true,
      },
      {
        key: "absence" as const,
        label: arabicSource("attendance.absence_excuse"),
        desc: arabicSource(
          "attendance.this_day_will_not_be_counted_as_an_absence",
        ),
        show:
          record?.status === arabicSource("common.absent") ||
          record?.rawStatus === "absent",
      },
    ],
    [record],
  );
  ...
  const visibleToggles = useMemo(
    () => excuseToggles.filter((toggle) => toggle.show),
    [excuseToggles],
  );

  const toggleExcuse = useCallback(
    (key: ExcuseKey): void => {
      onExcuseFormChange((current) => ({ ...current, [key]: !current[key] }));
    },
    [onExcuseFormChange],
  );
  ...
  const handleNoteChange = (
    ...
    onExcuseFormChange((current) => ({ ...current, note: value }));
  };
          ...
          <div className="p-5 space-y-4">
            <div className="flex flex-wrap gap-2" style={{ fontSize: 12 }}>
              <span className="px-2.5 py-1 rounded-full bg-primary/10 text-primary border border-primary/20">
                {arabicSource("attendance.attendance")}{" "}
                {excuseModal.record.checkIn || "—"}
              </span>
              <span className="px-2.5 py-1 rounded-full bg-primary/10 text-primary border border-primary/20">
                {arabicSource("attendance.dismissal")}{" "}
                {excuseModal.record.checkOut || "—"}
              </span>
              {excuseModal.record.lateMinutes > 0 && (
                <span className="px-2.5 py-1 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/20">
                  {arabicSource("attendance.delay")}{" "}
                  {excuseModal.record.lateMinutes} {arabicSource("common.min")}
                </span>
              )}
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder={arabicSource(
                  "attendance.enter_the_reason_for_the_excuse",
                )}
                rows={3}
          ...
          <div className="flex items-center justify-between p-5 border-t border-border/30 bg-muted/5">
            {excuseModal.record.excuseNote && (
              <p className="text-muted-foreground" style={{ fontSize: 11 }}>
                {arabicSource("attendance.latest_excuse")}{" "}
                {excuseModal.record.excuseNote.substring(0, 40)}
              </p>
            )}
