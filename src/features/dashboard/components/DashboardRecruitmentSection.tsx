...
const DashboardRecruitmentSection = ({
  data,
}: DashboardRecruitmentSectionProps) => {
  const {
    newHireStats,
    cfg,
    headcountTrend,
  ...
  const recruitmentStatCards = useMemo(
    () => [
      ...
      {
        label: arabicSource("common.total_applicants"),
        value: recruitmentStats.totalApplicants,
        sub: `${recruitmentStats.avgApplicantsPerJob} ${arabicSource("dashboard.average_function")}`,
        icon: Users,
        color: "text-blue-400",
      },
      {
        label: arabicSource("dashboard.average_time_to_hire"),
        value: `${recruitmentStats.avgTimeToFill} ${arabicSource("common.days_2")}`,
        sub: arabicSource("dashboard.from_applying_for_appointment"),
        icon: Clock,
        color:
          recruitmentStats.avgTimeToFill > cfg.timeToFillWarningDays
            ? "text-amber-400"
            : "text-emerald-400",
      },
      {
        label: arabicSource("dashboard.offer_acceptance_rate"),
        value: `${recruitmentStats.offerAcceptRate}%`,
        sub: `${recruitmentStats.hired} ${arabicSource("dashboard.were_appointed")}`,
        icon: Target,
        color: "text-emerald-400",
      },
      {
        label: arabicSource("dashboard.talent_bank"),
        value: recruitmentStats.bookmarked,
        sub: arabicSource("dashboard.saved_filter"),
        icon: Heart,
  ...
  const funnelStages: Array<{
    ...
    return recruitmentPipeline.map((stage: any, i: number) => ({
      ...
      conversionPct:
        i < recruitmentPipeline.length - 1 && stage.value > 0
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <DashboardChartCard className={cardCls}>
          ...
          {recruitmentPipeline.some((stage: any) => stage.value > 0) ? (
            ...
            <div className="flex items-center justify-center h-[280px] text-muted-foreground">
              {arabicSource("dashboard.no_employment_data")}
            </div>
          )}
        </DashboardChartCard>
        ...
        <DashboardChartCard className={cardCls}>
          <h3 className="text-foreground mb-4">
            {arabicSource("dashboard.hire_vs_exit")}
          </h3>
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              {hireVsExitTiles.map((tile) => (
                <ColorStatTile
                  key={tile.label}
                  padding="p-5"
                  valueTextClassName="text-3xl font-bold"
                  {...tile}
                />
              ))}
            </div>
            ...
            <div className="p-4 rounded-xl bg-muted/20">
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">
                  {arabicSource("dashboard.net_growth")}
                </span>
                <span
                  className={`text-lg font-bold ${netGrowth >= 0 ? "text-emerald-400" : "text-red-400"}`}
                >
                  {netGrowth >= 0 ? "+" : ""}
                  {netGrowth}
                </span>
            ...
            <div>
              <h4 className="text-sm text-muted-foreground mb-3">
                {arabicSource("dashboard.headcount_trend")}
              </h4>
              <CustomLineChart
                data={headcountTrend}
                color="#3B82F6"
                height={150}
                valueLabel={arabicSource("dashboard.number")}
              />
            </div>
          </div>
        </DashboardChartCard>
