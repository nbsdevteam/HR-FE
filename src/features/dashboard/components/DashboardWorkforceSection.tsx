...
const DashboardWorkforceSection = ({
  data,
}: DashboardWorkforceSectionProps) => {
  const {
    attendanceStats,
    turnoverRate,
    newHireStats,
  ...
  const turnoverTiles = useMemo(
    () => [
      {
        value: newHireStats.last30,
        label: arabicSource("dashboard.set_30_days"),
        colorClassName: "bg-emerald-500/10 border border-emerald-500/20",
        textColorClassName: "text-emerald-400",
      },
      {
        value: newHireStats.last90,
        label: arabicSource("common.set_90_days"),
        colorClassName: "bg-blue-500/10 border border-blue-500/20",
        textColorClassName: "text-blue-400",
      },
      {
        value: `${newHireStats.rate}%`,
        label: arabicSource("dashboard.assignment_rate"),
  ...
  const stats = useMemo(
    () => [
      {
        label: arabicSource("dashboard.average_service_life"),
        value: `${tenureStats.avg} ${arabicSource("common.years")}`,
        sub: `${arabicSource("dashboard.mediator")} ${tenureStats.median} ${arabicSource("common.years")}`,
        icon: Clock,
      },
      {
        label: arabicSource("dashboard.attendance_30_days"),
        value: `${attendanceStats.rolling30Rate}%`,
        sub: `${arabicSource("dashboard.absenteeism")} ${attendanceStats.absenteeismRate}%`,
        icon: ClipboardCheck,
      },
      {
        label: arabicSource("dashboard.discipline_ratio"),
        value: `${attendanceStats.punctualityRate}%`,
        sub: `${attendanceStats.late} ${arabicSource("dashboard.late_today")}`,
        icon: Target,
      },
      {
        label: arabicSource("common.active_contracts"),
        value: activeContracts,
        sub: `${probationCount} ${arabicSource("dashboard.in_the_experiment")}`,
        icon: Briefcase,
      },
      {
        label: arabicSource("common.use_of_vacations"),
        value: `${leaveUtilization.rate}%`,
        sub: `${leaveUtilization.totalUsed} ${arabicSource("common.from")} ${leaveUtilization.totalEntitled} ${arabicSource("common.days_2")}`,
        icon: CalendarDays,
      },
    ],
    [
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="text-center p-2 rounded-lg bg-amber-500/10">
                <p className="text-amber-400 font-medium">{pendingLeaves}</p>
                <p className="text-muted-foreground text-xs">
                  {arabicSource("common.pending_2")}
                </p>
              </div>
              <div className="text-center p-2 rounded-lg bg-emerald-500/10">
                <p className="text-emerald-400 font-medium">{approvedLeaves}</p>
                <p className="text-muted-foreground text-xs">
                  {arabicSource("dashboard.accepted")}
                </p>
              </div>
            ...
            <div className="p-3 rounded-lg bg-muted/20">
              <div className="flex justify-between text-sm mb-2">
                <span className="text-muted-foreground">
                  {arabicSource("common.use_of_vacations")}
                </span>
                <span className="text-primary font-medium">
                  {leaveUtilization.rate}%
                </span>
              </div>
              <DashboardMiniBar
                value={leaveUtilization.rate}
              ...
              />
              <p className="text-muted-foreground text-xs mt-1">
                {arabicSource("common.average")} {leaveUtilization.avgUsed}{" "}
                {arabicSource("dashboard.day_employee")}
              </p>
            </div>
          </div>
        </DashboardChartCard>
