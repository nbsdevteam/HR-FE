...
type TWarningEscalationPathProps = {
  warningTypes: string[];
};

const WarningEscalationPath = ({
  warningTypes,
}: TWarningEscalationPathProps) => (
  <motion.div
    ...
    <div className="flex items-start justify-between gap-4 overflow-x-auto pb-1">
      {warningTypes?.map((type, i) => (
        <WarningEscalationStep
          ...
          type={type}
          index={i}
          isLast={i === warningTypes.length - 1}
        />
      ))}
    </div>
  </motion.div>
...
export default memo(WarningEscalationPath);
